        ...
        let camTzTarget = 96; // Novo alvo de suavização de zoom
        const camLook = {
            x: 0,
            y: 0,
            z: 0
        ...
        // Lente Gravitacional: Alterado o brilho do anel para dourado/âmbar (Gargantua Style)
        const LensShader = {
            uniforms: {
                tDiffuse: {
                    value: null
        ...
        // Classificação OBAFGKM Realista (Estrelas Quentes/Azuis até Frias/Vermelhas)
        (function() {
            ...
            scene.add(new THREE.Points(mkLayer(6000, 1800, .2, .8, OBAFGKM[0], OBAFGKM[2]), mkMat({
                uT: {
                    value: 0
                }
            })));
            scene.add(new THREE.Points(mkLayer(3000, 1400, .25, .7, OBAFGKM[3], OBAFGKM[4]), mkMat({
                uT: {
                    value: 0
                }
            })));
        ...
        // Galáxia Espiral (Bulbo Amarelo + Braços Azuis)
        (function() {
            const ARMS = 4,
                N = 80000;
            ...
            for (let i = 0; i < N; i++) {
                ...
                const spiral = r * .52 + armAng,
                    spread = Math.pow(Math.random(), 1.8) * Math.max(0, (72 - r) * .18);
        ...
        // Buraco Negro (Blackbody Radiation - Branco p/ Âmbar/Ferrugem) Sem Azul
        (function() {
            scene.add(new THREE.Mesh(new THREE.SphereGeometry(2.2, 40, 40), new THREE.MeshBasicMaterial({
                color: 0x000000
            })));
            ...
            [
                ...
                const rm = new THREE.Mesh(new THREE.TorusGeometry(r, t, 12, 180), new THREE.MeshBasicMaterial({
                    color: c,
                    transparent: true,
                    opacity: o
                }));
            ...
            for (let i = 0; i < NA; i++) {
                const i3 = i * 3,
                    r = 2.6 + Math.pow(Math.random(), .5) * 11,
                    a = Math.random() * Math.PI * 2;
                aAng[i] = a;
                aRad[i] = r;
                ap[i3] = Math.cos(a) * r;
                ap[i3 + 1] = (Math.random() - .5) * .1 * (1 / (r * .15 + .9));
            ...
            for (let i = 0; i < NJ; i++) {
                const i3 = i * 3,
                    h = (Math.random() - .5) * 30,
                    r = Math.abs(h) * .035 * (Math.random() * .8 + .2),
                    a = Math.random() * Math.PI * 2;
                jp[i3] = Math.cos(a) * r;
                jp[i3 + 1] = h;
        ...
        function makePlanetTexture(pcol, catName, sz) {
            ...
            const {
                ...
                g,
                b
            } = hexToRgb(pcol), type = getPlanetType(catName);
            let seed = 0;
            ...
            if (type === 'gaseous' || type === 'nebular') {
                ...
                for (let i = 0; i < 8; i++) {
                    ctx.fillStyle = `rgba(255,255,255,${.08+rnd()*.12})`;
                    ctx.fillRect(0, rnd() * sz, sz, 2 + rnd() * sz * .08);
                }
                ...
                for (let i = 0; i < 10; i++) {
                    ...
                    ctx.beginPath();
                    ctx.arc(rnd() * sz, rnd() * sz, rnd() * sz * .2, 0, Math.PI * 2);
                    ctx.fill();
                }
            }
        ...
        uniqCats.forEach((name, idx) => {
            ...
            const star = new THREE.Mesh(new THREE.SphereGeometry(.7, 24, 24), new THREE.MeshBasicMaterial({
                color: col
            }));
            ...
            glow.material.opacity = .9;
            glow2.position.set(x, y, z);
            glow2.material.opacity = .24;
            neb.position.set(x, y, z);
            neb.material.opacity = .065;
            scene.add(glow);
            scene.add(glow2);
            scene.add(neb);
            catMap.set(name, {
        ...
        const MAX_CONST = 200;
        const constPos = new Float32Array(MAX_CONST * 6);
        ...
        const constLines = new THREE.LineSegments(constGeom, new THREE.LineBasicMaterial({
            color: 0x2AA2F6,
            transparent: true,
            opacity: .1,
            depthWrite: false,
        ...
        LIVROS.forEach((livro, idx) => {
            ...
            const mesh = new THREE.Mesh(new THREE.SphereGeometry(1, 20, 20), new THREE.MeshBasicMaterial({
                map: tex
            }));
            ...
            const oLine = new THREE.LineLoop(oGeom, new THREE.LineBasicMaterial({
                color: pcol,
                transparent: true,
                opacity: .1,
                depthWrite: false
        ...
        const sysMenu = document.getElementById('systems-menu');
        ...
        // Atalho ESCAPE para UX Premium
        document.addEventListener('keydown', e => {
            if (e.key === 'Escape') {
                if (document.getElementById('modal-overlay').classList.contains('open')) {
                    document.getElementById('modal-overlay').classList.remove('open');
                } else if (document.getElementById('book-panel').classList.contains('open')) {
                    hidePanel();
        ...
        // Clique no espaço vazio também fecha o painel
        ...
        function openModal(d) {
            ...
            mc.height = 240;
            drawBookArt(mc, d.titulo, d.categoria_nome, mc.width, mc.height);
        ...
        function flyToCat(name) {
            ...
            if (name !== 'all' && activeView === 'gal') {
                ...
                cp.y += 16;
                cp.x += 6;
                camTzTarget = cp.z; // Atualiza o alvo do scroll para manter consistência
                ...
                catMap.forEach(cd => gsap.to(cd.star.scale, {
                    x: 1,
                    y: 1,
                    z: 1,
                    duration: .7
        ...
        function buildEditorial() {
            ...
            if (filtered.length) {
                const hero = filtered[0],
                    hc = document.getElementById('ed-hero-canvas');
        ...
        (function animate(now) {
            ...
            bookObjs.forEach(b => {
                ...
                if (b.filtered && b.searched) {
                    ...
                    b.mesh.visible = b.currentScale > .02;
                } else {
                    b.mesh.visible = false;
                }
                if (b.glow.visible) {
            ...
            catMap.forEach(cd => {
                ...
                cd.glow.material.opacity = p * .88;
                cd.glow2.material.opacity = p * .22;
                cd.neb.material.opacity = .058 + .025 * Math.sin(t * .55 + cd.pos.z * .07);
                cd.star.rotation.y = t * .2;
                cd.star.scale.setScalar(.95 + .05 * Math.sin(t * 2 + cd.pos.x * .15));
            });
        ...
        gsap.from('#scroll-hint', {
            ...
            duration: 1,
            delay: INTRO_DUR + 1.2,
            onComplete: () => gsap.to('#scroll-hint', {
                opacity: 0,
                delay: 4,
                duration: 1.6
            })
