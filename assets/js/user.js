...
(function () {
  ...
  andromedaReady(() => {
    ...
    let perfilEcoMode = localStorage.getItem(ECO_STORAGE_KEY) === "1" || window.ANDROMEDA_PROFILE_ECO === true;
    const LEGACY_KEYS = [
    ...
    function savedMode() {
      const bootMode = normalizeMode(window.ANDROMEDA_PROFILE_MODE);
      if (bootMode) return bootMode;
    ...
    let quality = QUALITY_PROFILES[qualityName] || QUALITY_PROFILES.cinema;
    perfilEcoMode = qualityName === "eco";
    ...
    document.body.classList.add(`perfil-mode-${qualityName}`);
    document.body.classList.toggle("perfil-eco-mode", perfilEcoMode);
    document.body.classList.toggle("perfil-animations-active", !perfilEcoMode);
    localStorage.setItem(ECO_STORAGE_KEY, perfilEcoMode ? "1" : "0");
    localStorage.setItem(QUALITY_KEY, qualityName);
    ...
    function createQualityButton() {
      ...
      btn.setAttribute("aria-label", "Alternar qualidade visual do perfil");
      btn.innerHTML = `
        <span class="andromeda-quality-dot" aria-hidden="true"></span>
        <span class="andromeda-quality-label"></span>
        <span class="andromeda-quality-next"></span>
      `;
      ...
      function refresh() {
        ...
        const nextLabel = btn.querySelector(".andromeda-quality-next");
        btn.dataset.mode = qualityName;
        btn.title = `Modo ${QUALITY_PROFILES[qualityName].label}. Clique para alternar para ${QUALITY_PROFILES[next].label}.`;
        if (label) label.textContent = `Perfil · ${QUALITY_PROFILES[qualityName].label}`;
        if (nextLabel) nextLabel.textContent = `→ ${QUALITY_PROFILES[next].label}`;
      }

      btn.addEventListener("click", () => {
        const next = nextMode(qualityName);
        localStorage.setItem(QUALITY_KEY, next);
        localStorage.setItem(ECO_STORAGE_KEY, next === "eco" ? "1" : "0");
    ...
    function setupCursor() {
      ...
      if (!reticle || isTouch || perfilEcoMode) return;
      ...
      let raf = 0;
      const selector =
        "a, button, input, textarea, select, label, .nav-link, .nav-item, .btn, .form-control, .interactable, .andromeda-quality-toggle";

      function schedule() {
        if (!raf && visible) raf = requestAnimationFrame(cursorLoop);
      }

      function cursorLoop() {
        raf = 0;
        if (!visible || perfilEcoMode) return;
        const retSpeed = hovering ? 0.46 : 0.34;
        rx += (tx - rx) * retSpeed;
        ry += (ty - ry) * retSpeed;
        dx += (tx - dx) * 0.86;
        dy += (ty - dy) * 0.86;
        ...
        if (Math.abs(tx - rx) + Math.abs(ty - ry) + Math.abs(tx - dx) + Math.abs(ty - dy) > 0.18) {
          schedule();
        }
      }
      ...
      document.addEventListener(
        ...
        (e) => {
          tx = e.clientX;
          ty = e.clientY;
          const nextHover = Boolean(e.target.closest?.(selector));
          if (nextHover !== hovering) {
            hovering = nextHover;
            reticle.classList.toggle("hover", hovering);
          }
          schedule();
        },
        { passive: true },
    ...
    function setupUIAnimations() {
      if (perfilEcoMode) {
        document.querySelectorAll(".perfil-header, .nav-tabs, .form-card, .alert").forEach((el) => {
          el.style.opacity = "";
          el.style.transform = "";
          el.style.filter = "";
        });
...
(function () {
  document.addEventListener("DOMContentLoaded", function () {
    ...
    input.addEventListener("change", function () {
      ...
      preview.innerHTML = `<img src="${url}" alt="Prévia da foto">`;
    });
  });
})();
...
(function () {
  ...
  function activatePerfilTab(target) {
    if (!target || typeof bootstrap === "undefined" || !bootstrap.Tab) return;
    const trigger = document.querySelector(`[data-bs-target="${target}"]`);
    if (trigger) new bootstrap.Tab(trigger).show();
  }

  document.addEventListener("DOMContentLoaded", () => {
    const reservasLink = document.getElementById("perfil-nav-reservas");
    if (reservasLink) {
      reservasLink.addEventListener("click", (event) => {
        event.preventDefault();
        activatePerfilTab("#tab-reservas");
    ...
    if (["#tab-reservas", "#tab-avaliacoes", "#tab-ranking", "#tab-seguranca"].includes(hash)) {
      setTimeout(() => activatePerfilTab(hash), 80);
    }

    const inputAvatar = document.getElementById("avatar");
    const avatarCopy = document.querySelector(".perfil-avatar-copy p");
    if (inputAvatar && avatarCopy) {
      inputAvatar.addEventListener("change", () => {
        const file = inputAvatar.files && inputAvatar.files[0];
        if (!file) return;
        avatarCopy.textContent = file.name;
      });
    }
  });
})();
...
(function () {
  ...
  ready(() => {
    if (document.body.classList.contains("perfil-eco-mode")) return;
    if (typeof gsap === "undefined") return;
    ...
    const animateActiveTab = () => {
      ...
      gsap.fromTo(active, { y: 4 }, { y: 0, duration: 0.42, ease: "power3.out", overwrite: true });
    };
    ...
    document.querySelectorAll('[data-bs-toggle="tab"]').forEach((tab) => {
      tab.addEventListener("shown.bs.tab", () => {
        ...
        gsap.fromTo(
          target.querySelectorAll(".form-card, .perfil-reserva-card, .perfil-avaliacao-card, .perfil-ranking-card, table tbody tr"),
          { autoAlpha: 0, y: 18, filter: "blur(8px)" },
          { autoAlpha: 1, y: 0, filter: "blur(0px)", duration: 0.55, stagger: 0.04, ease: "power3.out", overwrite: true },
        );
      });
    });

    document.addEventListener("pointerover", (event) => {
      const card = event.target.closest?.(".form-card, .perfil-reserva-card, .perfil-avaliacao-card, .perfil-ranking-card, .perfil-orbit-stats span");
      if (!card || reduce) return;
      gsap.to(card, { y: -4, scale: 1.006, duration: 0.28, ease: "power2.out", overwrite: true });
    }, true);

    document.addEventListener("pointerout", (event) => {
      const card = event.target.closest?.(".form-card, .perfil-reserva-card, .perfil-avaliacao-card, .perfil-ranking-card, .perfil-orbit-stats span");
      if (!card || reduce) return;
      gsap.to(card, { y: 0, scale: 1, duration: 0.36, ease: "power2.out", overwrite: true });
    }, true);
    ...
    if (avatarInput && avatarPreview) {
      avatarInput.addEventListener("change", () => {
        if (reduce) return;
        gsap.fromTo(avatarPreview, { scale: 0.92, rotate: -2, filter: "brightness(1.35) blur(3px)" }, { scale: 1, rotate: 0, filter: "brightness(1) blur(0px)", duration: 0.58, ease: "back.out(1.8)", overwrite: true });
      });
    }
