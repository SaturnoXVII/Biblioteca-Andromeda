            ...
            if ($result->num_rows === 1) {
                ...
                if (password_verify($senha, $usuario['senha'])) {
                    ...
                    if ($usuario['nivel_acesso'] === 'admin') {
                        header("Location: /Biblioteca-Andromeda/adm");
                        exit;
                    }
...
<head>
    ...
    <link rel="stylesheet" href="/Biblioteca-Andromeda/assets/css/pglogin.css">
...
<body>
    ...
    <main class="auth-wrapper">
        ...
        <div class="auth-card" id="Cadastro">
            ...
            <form method="POST" action="/Biblioteca-Andromeda/login" novalidate>
                ...
                <div class="form-group">
                    <input type="email" name="email" id="campo_login_email" placeholder="E-mail" required class="form-control interactable">
                </div>

                <div class="form-group pass-wrap">
                    <input type="password" name="senha" id="campo_login_senha" placeholder="Senha" required class="form-control interactable">
                ...
                <div class="auth-links" id="adicionais">
                    <a href="/Biblioteca-Andromeda/cadastro-leitores" class="interactable">Não tem uma conta?</a>
                </div>
            </form>
        </div>
    </main>
    ...
    <script src="/Biblioteca-Andromeda/assets/js/animelogin.js"></script>
    ...
    function marcarCampoLoginInvalido(campo, mensagem) {
        document.querySelectorAll(".is-invalid-cosmic").forEach(item => item.classList.remove("is-invalid-cosmic"));
    ...
    document.addEventListener("DOMContentLoaded", () => {
        ...
        form.addEventListener("submit", (event) => {
            const email = document.getElementById("campo_login_email");
            const senha = document.getElementById("campo_login_senha");

            if (!email.value.trim()) {
                event.preventDefault();
                marcarCampoLoginInvalido(email, "Informe seu e-mail para acessar o acervo.");
                return;
            }
            ...
            if (!emailValido) {
                event.preventDefault();
                marcarCampoLoginInvalido(email, "Informe um e-mail válido.");
                return;
            }

            if (!senha.value.trim()) {
                event.preventDefault();
                marcarCampoLoginInvalido(senha, "Informe sua senha para continuar.");
                return;
            }
    ...
    function handleCredentialResponse(response) {
        ...
        .then(data => {
            if (data.status === "success") {
                andromedaNotify("Acesso confirmado. Abrindo o universo Andrômeda...", "success");
                window.setTimeout(() => {
                    if (data.nivel_acesso === "admin") {
                        window.location.href = "/Biblioteca-Andromeda/adm";
                    } else {
                        window.location.href = "/Biblioteca-Andromeda/catalogo";
                    }
                }, 900);
            } else if (data.status === "not_registered") {
                andromedaNotify("Você ainda não tem uma conta. Faça seu cadastro primeiro.", "error");
            } else {
                andromedaNotify("Erro no login com Google. Tente novamente.", "error");
            }
        })
        .catch(err => {
